...
function filepath.glob()
{
	...
	if [ -x "${1%/*}" ]; then	
		...
		error.__trace arg 'path' 'dir' "${1%/*}" "$__FILEPATH_ERR_READ_DIR"
	fi
...
function filepath.copy()
{
	...
	if [ -w "$2" ]; then
		case $3 in
			0) return 0;;
			1) ;;
			*) err=1; error.__trace arg 'override' 'uint' "$3" 'flag inválida';;
		esac
		...
		if [ ! "$err" ]; then
			...
			error.__trace arg 'dest' 'dir' "$1" "$__FILEPATH_ERR_COPY_PATH"
		fi
	else	
		error.__trace arg 'dest' 'dir' "$2" "$__FILEPATH_ERR_WRITE_DENIED"
	fi
...
function filepath.diff()
{
	...
	if [[ ! -r "$1" ]]; then
		error.__trace arg 'file1' 'file' "$1" "$__FILEPATH_ERR_READ_DENIED"
	elif [[ ! -r "$2" ]]; then
		error.__trace arg 'file2' 'file' "$2" "$__FILEPATH_ERR_READ_DENIED"
	else
		mapfile -t f1 < "$1"
		mapfile -t f2 < "$2"
