...
function time.tzinfo()
{
	...
	if [[ ${__timezones[$tzname]} ]]; then
		...
		error.__trace arg "tzname" "str" "$tzname" "$__TIME_ERR_TZNAME"
	fi
...
{
	[[ -e $__TIME_TZFILE ]] || error.__trace arg '' '' '' "$__TIME_ERR_TZFILE '$__TIME_TZFILE'"
	[[ -e $__TIME_CTZFILE ]] || error.__trace arg '' '' '' "$__TIME_ERR_TZFILE '$__TIME_CTZFILE'"	
