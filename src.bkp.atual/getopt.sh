...
function getopt.parse()
{
	...
	do
		...
		[[ $name =~ ^(${names%|})$ ]] && error.__trace arg "$name" '' "$value" "$__GETOPT_ERR_NAME_CONFLICT"
		[[ $name =~ ^[a-zA-Z0-9_-]+$ ]] || error.__trace arg "$name" '' "$value" "$__GETOPT_ERR_PARAM_NAME"

		case $flag in
			+)	[[ $value ]] || error.__trace arg "$name" "$ctype" '<null>' "$__GETOPT_ERR_ARG_REQUIRED";;
			-)	[[ $value ]] || continue;;
			*)	error.__trace arg "$name" '' "${flag:-<null>}" "$__GETOPT_ERR_FLAG";;
		esac
		...
		case $ctype in
			...
			keyword) [[ $value == $name ]];;
			null) error.__trace arg "$name" "$ctype" "$value" "$__GETOPT_ERR_NOT_ARG";;
			dir) [[ -d $value ]] || error.__trace arg "$name" "$ctype" "$value" "$__GETOPT_ERR_DIR_NOT_FOUND";;
			file) [[ -f $value ]] || error.__trace arg "$name" "$ctype" "$value" "$__GETOPT_ERR_FILE_NOT_FOUND";;
			path) [[ -e $value ]] || error.__trace arg "$name" "$ctype" "$value" "$__GETOPT_ERR_PATH_NOT_FOUND";;
			fd) ([[ $value =~ ^(0|[1-9][0-9]*)$ ]]; [[ -e /dev/fd/$value ]]) || \
				error.__trace arg "$name" "$ctype" "$value" "$__GETOPT_ERR_FD_NOT_EXISTS";;
			type) printf -v obj_types '%s|' ${!__BUILTIN_TYPE_IMPLEMENTS[@]}${__INIT_TYPE_IMPLEMENTS[@]:+ ${!__INIT_TYPE_IMPLEMENTS[@]}}
				  [[ "$value" =~ ^(${obj_types%|})$ ]] || error.__trace arg "$name" "$ctype" "$value" "$__GETOPT_ERR_VAR_TYPE";;

			*) error.__trace arg "$name" "$ctype" '' "$__GETOPT_ERR_TYPE_PARAM '$ctype'";;
       	esac

		(($?)) && error.__trace arg "$name" "$ctype" "$value" "$__GETOPT_ERR_TYPE_ARG $ctype"
