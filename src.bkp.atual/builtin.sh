...
function fndef()
{
	...
	if which $2 &>/dev/null || declare -fp $2 &>/dev/null; then
		error.__trace arg "newtype" "funcname" "$2" "$__BUILTIN_ERR_FUNC_EXISTS"
	...
	while read iter; do
		for cond in "${@:3}"; do
			if [[ $cond =~ $re ]]; then
				...
				error.__trace arg 'cond' 'str' "$cond" 'instrução condicional inválida'
				return 1
			fi
		done
...
function builtin.__init()
{
	...
	if ! mkdir -p "$__RUNTIME" &>/dev/null; then
		error.__trace arg '' '' "$__RUNTIME" 'não foi possível gerar os arquivos temporários'
	fi
