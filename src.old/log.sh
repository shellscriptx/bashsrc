...
function log.__format()
{
	...
	case $flag in
		...
		5) fmt='%(%d%m%Y%H%M%S)T';;
		6) fmt='%(%s)T';;
		*) error.__trace def "$1" 'flag' "$flag" "$__ERR_LOG_FLAG"; return $?;;
	esac
	...
	if [[ $logfile ]]; then
		if [[ -e "$logfile" && ! -f "$logfile" ]]; then
			error.__trace def 'struct' 'file' "$logfile" "$__ERR_LOG_NOFILE"
			return $?
		elif [[ -e "$logfile" && ! -w "$logfile" ]]; then
			error.__trace def 'struct' 'file' "$logfile" "$__ERR_LOG_NOWFILE"
			return $?
		fi
	fi
	...
	printf '%s: %s: %s: %s: %s\n'  	"${0##*/}" \
		...
		error.__trace def 'struct' 'file' "$logfile" "$__ERR_LOG_WFILE"
		return $?
	}
