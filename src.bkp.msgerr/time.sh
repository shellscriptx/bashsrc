...
function time.tzinfo()
{
	...
	if [[ ${__timezones[$tzname]} ]]; then
		...
		error.__trace arg "tzname" "str" "$tzname" "$__TIME_ERR_TZNAME"; return $?
	fi
...
function time.asctime()
{
	...
	if ! (time.__check_time ${__asctime_map[tm_hour]} \
		...
		error.__trace arg 'datetime' 'map' "\n$(map.list $1)" "$__TIME_ERR_DATETIME"; return $?
	fi
...
function time.strftime()
{
	...
	if ! (time.__check_time ${__dt_ref[tm_hour]} \
		...
		error.__trace arg 'datetime' 'map' "\n$(map.list $2)" "$__TIME_ERR_DATETIME"; return $?
	fi
...
function time.__init()
{
	if [[ ! -e $__TIME_TZFILE ]]; then
		error.__trace arg '' '' '' "$__TIME_ERR_TZFILE '$__TIME_TZFILE'"; return $?
	elif [[ ! -e $__TIME_CTZFILE ]]; then
		error.__trace arg '' '' '' "$__TIME_ERR_TZFILE '$__TIME_CTZFILE'"; return $?
	fi
