...
readonly __BUILTIN_ERR_TYPE_CONFLICT='conflito de tipos: o tipo especificado já foi inicializado'
readonly __BUILTIN_ERR_METHOD_NOT_FOUND='o método de implementação não existe'
readonly __BUILTIN_ERR_METHOD_CONFLICT='conflito de métodos: o método já foi implementado ou é uma função reservada'
readonly NULL=0
...
function fndef()
{
	...
	if which $2 &>/dev/null || declare -fp $2 &>/dev/null; then
		error.__trace arg "newtype" "funcname" "$2" "$__BUILTIN_ERR_FUNC_EXISTS"; return $?
	...
	while read iter; do
		for cond in "${@:3}"; do
			if [[ $cond =~ $re ]]; then
				...
				error.__trace arg 'cond' 'str' "$cond" 'instrução condicional inválida'; return $?
				return 1
			fi
		done
			...
			for method in	${__BUILTIN_TYPE_IMPLEMENTS[$type]} \
				...
				if ! [[ $var.${method##*.} =~ ^(${regmethods%|})$ ]]; then
					...
					if struct_func=$(declare -fp $method 2>/dev/null); then
						...
						error.__trace imp "$var" "$type" "$method" "$__BUILTIN_ERR_METHOD_NOT_FOUND"; return $?
					fi
				else
					error.__trace imp "$var" "$type" "$method" "$__BUILTIN_ERR_METHOD_CONFLICT"; return $?
				fi
			done
			eval "$var.__type__(){ echo $type; return 0; }"
			__REG_LIST_VAR[${FUNCNAME[1]}.$var]+="$var.__type__ "
		else
			error.__trace arg 'varname' 'var' "$var" "$__BUILTIN_ERR_ALREADY_INIT"; return $?
		fi
	done
		...
		if [[ "$attr" =~ r ]]; then
			...
			for type in ${!SRC_TYPE_IMPLEMENTS[@]}; do
				if [[ $type =~ ^(${regtypes%|})$ ]]; then
					error.__trace src '' "${BASH_SOURCE[-2]}" "$type" "$__BUILTIN_ERR_TYPE_CONFLICT"; return $?
				else
					for method in ${SRC_TYPE_IMPLEMENTS[$type]}; do
						if ! readonly -f $method 2>/dev/null; then
							error.__trace imp '' "$type" "$method" "$__BUILTIN_ERR_METHOD_NOT_FOUND"; return $?
						fi
					done
...
function builtin.__init()
{
	...
	if ! mkdir -p "$__RUNTIME" &>/dev/null; then
		error.__trace arg '' '' "$__RUNTIME" 'não foi possível gerar os arquivos temporários'; return $?
	fi
