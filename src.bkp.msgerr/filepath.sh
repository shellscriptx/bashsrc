...
function filepath.glob()
{
	...
	if [ -x "${1%/*}" ]; then	
		...
		error.__trace arg 'path' 'dir' "${1%/*}" "$__FILEPATH_ERR_READ_DIR"; return $?
	fi
...
function filepath.copy()
{
	...
	if [ -w "$2" ]; then
		case $3 in
			0) return 0;;
			1) ;;
			*) err=1; error.__trace arg 'override' 'uint' "$3" 'flag inválida'; return $?;;
		esac
		...
		if [ ! "$err" ]; then
			...
			error.__trace arg 'dest' 'dir' "$1" "$__FILEPATH_ERR_COPY_PATH"; return $?
		fi
	else	
		error.__trace arg 'dest' 'dir' "$2" "$__FILEPATH_ERR_WRITE_DENIED"; return $?
	fi
...
function filepath.diff()
{
	...
	if [[ ! -r "$1" ]]; then
		error.__trace arg 'file1' 'file' "$1" "$__FILEPATH_ERR_READ_DENIED"; return $?
	elif [[ ! -r "$2" ]]; then
		error.__trace arg 'file2' 'file' "$2" "$__FILEPATH_ERR_READ_DENIED"; return $?
	else
		mapfile -t f1 < "$1"
		mapfile -t f2 < "$2"
