...
function os.chmod()
{
	...
	if ! [[ $2 =~ ^[0-7]{3,4}$ ]]; then
		error.__trace arg 'mode' 'uint' "$2" "$__OS_ERR_MODE_PERM"; return $?
	fi
	chmod "$2" "$1" &>/dev/null
	return $?
}
...
function os.stackdir()
{
	...
	[ ! -d "$__dir" ] && error.__trace arg 'dir' 'str' "$__dir" "$__OS_ERR_DIR_NOT_FOUND"; return $?
	__stack_dir+=("$__dir")
...
function os.__chtime()
{
	...
	if ! (time.__check_time ${__map_ref[tm_hour]} \
        ...
        error.__trace arg 'time' 'map' "\n$(map.list $3)" "$__TIME_ERR_DATETIME"; return $?
    fi
...
function os.open()
{
	...
	if [ ! -d /dev/fd ]; then
		error.__trace arg '' '' '' "'/dev/fd' diretório FIFOs para método I/O não encontrado"; return $?
	elif [ -d "$__file" ]; then
		error.__trace arg 'filename' 'str' "$__file" 'é um diretório'; return $?
	fi
	...
	if [ $__av -eq 0 ]; then
		error.__trace arg 'file' 'fd' "$__file" "$__OS_ERR_FD_OPEN_MAX"; return $?
	fi
	
	case $__mode in
		0)
			if [[ ! -e "$__file" ]]; then
				error.__trace arg 'file' 'str' "$__file" "$__OS_ERR_FILE_NOT_FOUND"; return $?
			elif [[ ! -r "$__file" ]]; then
				error.__trace arg 'file' 'str' "$__file" "$__OS_ERR_FILE_NOT_READ"; return $?
			fi
			__parse="$__fd<$__file"
			;;
		...
		1)
			if [[ -e "$__file" && ! -w "$__file" ]]; then
				error.__trace arg 'file' 'str' "$__file" "$__OS_ERR_FILE_NOT_WRITE"; return $?
			fi
			__parse="$__fd>>$__file"
			;;
		...
		2) 
			if [[ -e "$__file" ]] && [[ ! -w "$__file" || ! -r "$__file" ]]; then
				error.__trace arg 'file' 'str' "$__file" "$__OS_ERR_FILE_NOT_RW"; return $?
			fi
			__parse="$__fd<>$__file"
			;;
		*) error.__trace arg 'flag' 'uint' "$__mode" "$__OS_ERR_OPEN_FLAG"; return $?;;
	esac

	eval exec "$__parse" 2>/dev/null || \
	error.__trace arg 'descriptor' "fd" '-' "$__OS_ERR_FD_CREATE '$__fd'"; return $?
...
function os.file.readlines()
{
	...
	error.__trace arg 'descriptor' "fd" '-' "$__OS_ERR_FD_READ '$1'"; return $?
...
function os.file.writeline()
{
	...
	echo "$2" >&$1 2>/dev/null || \
	error.__trace arg 'descriptor' "fd" '-' "$__OS_ERR_FD_WRITE '$1'"; return $?
...
function os.file.seek()
{
	...
	case $mode in
		...
		1) 	parse="$fd>>$file";;
		2)	parse="$fd<>$file";;
		*) 	error.__trace arg 'flag' 'uint' "$mode" "$__OS_ERR_OPEN_FLAG"; return $?;;
	esac
	
	eval exec "$parse" 2>/dev/null || error.__trace arg 'descriptor' "fd" '-' "$__OS_ERR_FD_READ '$fd'"; return $?

	case $whence in
		0)	os.file.read $fd $offset 1>/dev/null;;
		1) 	os.file.read $fd $((cur+offset)) 1>/dev/null;;
		2)	os.file.read $fd $end 1>/dev/null;;
		*) 	error.__trace arg 'whence' 'uint' "$whence" "$__OS_ERR_SEEK_FLAG"; return $?;;
	esac
