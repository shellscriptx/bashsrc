...
function fnmap(){
	...
	getopt.parse -1 "name:var:+:$1" "funcname:func:+:$2"
...
function fnrange()
{
	getopt.parse -1 "min:int:+:$1" "max:int:+:$2" "step:int:+:$3" "funcname:func:+:$4"
...
function fniter()
{
	getopt.parse -1 "iterable:str:-:$1" "funcname:func:+:$2"
...
function all()
{
	getopt.parse -1 "iterable:str:-:$1" "cond:str:+:$2"
...
function var()
{
	getopt.parse -1 "varname:var:+:$1" "type:type:+:${@: -1}"
	...
	for var in ${@:1:$((${#@}-1))}; do
		...
		for method in	${__BUILTIN_TYPE_IMPLEMENTS[$type]} \
			...
			ptr_func="^\s*${method//./\\.}\s*\(\)\s*\{\s*getopt\.parse\s+-?[0-9]+\s+[\"'][a-zA-Z_]+:(var|map|array|func):[+-]:[^\"']+[\"']"
...
{
	getopt.parse 2 "var:var:+:$1" "funcname:func:+:$2" ${@:3}
